...
import mockOwnPackages from "../../../../mockData/mockOwnSectionPackages";
...
const FormOne = ({setFormOne}) => {
    ...
    return (
        <>
            ...
            <div onChange={(event) => setOption(event)} className="modal_radio_container">
                ...
                <div className="form1_divider" />
                <div className="form1_input">
                    <input type="radio" value="TRANSFER" name="form1" /> I want to tranfer my travel blog to Globuzzer
                </div>
                <div className="form1_divider" />
                <div className="form1_input">
                    <input type="radio" value="OTHER" name="form1" /> Other (please, exlain in the field below)
                </div>
                <textarea id="other_text_area" name="other_text_area" rows="4" cols="50" spellCheck />
            </div>
        </>
    );
};
...
const FormTwo = ({
    formTwo, handleFormTwo, isAgreed, handlePolicy
}) => {
    const {
...
const FormThree = ({width}) => (
    <div className="form_3">
        {mockOwnPackages.map((pricingPackage, index) => (
            <OwnSectionPlan key={index} pricingPackage={pricingPackage} width={width} />
        ))}
    </div>
);
export const GetStarted = ({modalIsOpen, setModalIsOpen}) => {
    ...
    const formTwoCondition = formStage === 1 && (firstname === "" || lastname === "" || email === "" || phone === "" || isAgreed === false);
    ...
    const styles = {
        circle1: {backgroundColor: !formOneCondition && "white", color: !formOneCondition && "#f24b6a"},
        circle2: {
    ...
    return (
        ...
        >
            <div className="modal_header">
                <FiX onClick={() => setModalIsOpen(!modalIsOpen)} className="modal_close_icon" />
                {formStage === 3 ? (
            ...
            {formStage !== 3 && <p className="modal_required">* yes, you have to fill out these fields</p>}
            ...
            {formStage !== 3 && (
                <button type="button" onClick={() => nextForm()} className="modal_button_continue">
                    Continue
                </button>
            )}
            {formStage === 3 && (
                <>
                    ...
                    <p id="final_des">
                        Thanks for applying! We have sent you a confirmation message to your e-mail address. We will
                        contact you as soon as possible.
                    </p>
                    <button type="button" onClick={() => setModalIsOpen(!modalIsOpen)} className="final_button">
                        Close
                    </button>
                </>
            )}
